...
/**
 * A copy of the JSR356AsyncSupport Atmosphere file. The class is identical to the original, however the {@link AtmosphereConfigurator} is patched to
 * work around https://github.com/mvysny/vaadin-quarkus/issues/17. See {@link AtmosphereConfigurator} for more details. Also see
...
public class JSR356AsyncSupportPatch extends Servlet30CometSupport {
    ...
    public JSR356AsyncSupportPatch(final AtmosphereConfig config) {

        this(config, config.getServletContext());
    }

    public JSR356AsyncSupportPatch(final AtmosphereConfig config, final ServletContext ctx) {

        super(config);
        // get the server-container from Quarkus container-provider. original it's getting by ctx-attribute
        final ServerContainer container = (ServerContainer) ContainerProvider.getWebSocketContainer();
        ...
        int pathLength = 5;
        final String s = config.getInitParameter(ApplicationConfig.JSR356_PATH_MAPPING_LENGTH);
        if (s != null) {
        ...
        final StringBuilder b = new StringBuilder(servletPath);
        final List<String> endpointPaths = new ArrayList<>();
        ...
        for (final String endpointPath : endpointPaths) {
            ...
            try {
                final ServerEndpointConfig endpointConfig = ServerEndpointConfig.Builder.create(JSR356Endpoint.class, endpointPath)
                        .configurator(this.configurator).build();
                container.addEndpoint(endpointConfig);
            } catch (final DeploymentException e) {
                logger.warn("Duplicate Servlet Mapping Path {}. Use {} init-param to prevent this message", servletPath,
    ...
    public final static class AtmosphereConfigurator extends ServerEndpointConfig.Configurator {
        ...
        /**
         * TODO: UGLY! Quarkus call modifyHandshake BEFORE getEndpointInstance() where other jsr356 do the reverse. However, because of VertX the
         * thread pool of Undertow handling regular http requests differ to the thread pool handling websocket requests, so we can't use ThreadLocals
         * to remember the HandshakeRequest. Use BlockingDeque with the capacity of 1 since we don't want the requests to get mixed up in case of high
         * concurrency.
         *
         * The reasoning for BlockingDeque is that we expect that a call to getEndpointInstance() will immediately follow call to modifyHandshake().
         * We will therefore store the request into this deque, and we will pick it up in getEndpointInstance().
         *
         * The deque has a capacity of 1: if there are any concurrent attempts to initialize the endpoint, any follow-up modifyHandshake() will be
         * delayed until the getEndpointInstance() is called.
         *
        ...
        public AtmosphereConfigurator(final AtmosphereFramework framework) {
        ...
        public <T> T getEndpointInstance(final Class<T> endpointClass) throws InstantiationException {
            ...
            if (JSR356Endpoint.class.isAssignableFrom(endpointClass)) {
                final JSR356Endpoint e = new JSR356Endpoint(this.framework,
                        WebSocketProcessorFactory.getDefault().getWebSocketProcessor(this.framework));
                final HandshakeRequest request = this.requests.removeFirst();
        ...
        public void modifyHandshake(final ServerEndpointConfig sec, final HandshakeRequest request, final HandshakeResponse response) {
            ...
            try {
                ...
                if (!this.requests.offer(request, 1, TimeUnit.SECONDS)) {
